...
typedef enum { NOT_DUG, DUG, PIN } t_ground;

typedef enum { NONE, DIG_BOM, CLEAR } t_game_endings;

// y,x
const t_pos VER_HORI_OBLIQUE[8] = {{-1, 0}, {-1, 1}, {0, 1},   {1, 1},
                                   {1, 0},  {1, -1}, {-1, -1}, {0, -1}};
        ...
        for (size_t j = 0; j < BD_WD; j++) {
            if (g_under_bd[i][j] == BOM && has_all_bom_show)
                buf_i += push_buf(&buf[buf_i],
                                  IS_CURSOR ? "\x1b[7m\x1b[31mB" : "\x1b[31mB");
            else if (g_ground_bd[i][j] == NOT_DUG)
                buf_i += push_buf(&buf[buf_i],
                                  IS_CURSOR ? "\x1b[33mO" : "\x1b[7m\x1b[33mO");
            else if (g_ground_bd[i][j] == PIN)
                buf_i += push_buf(&buf[buf_i], IS_CURSOR ? "\x1b[7m\x1b[35mP"
                                                         : "\x1b[43m\x1b[35mP");
            else if (g_under_bd[i][j] == BOM)
                buf_i += push_buf(&buf[buf_i],
                                  IS_CURSOR ? "\x1b[7m\x1b[31mB" : "\x1b[31mB");
            else if (g_under_bd[i][j] >= 0 && g_under_bd[i][j] <= 8) {
                sprintf(&buf[buf_i], IS_CURSOR ? "\x1b[7m%d" : "\x1b[37m%d",
                        g_under_bd[i][j]);
                buf_i += IS_CURSOR ? 5 : 6;
            } else {
...
void add_bom() {
    for (size_t i = 0; i < N_MINE; i++) {
        ...
        while (g_under_bd[h][w] == BOM) h = rand() % BD_HT, w = rand() % BD_WD;
        g_under_bd[h][w] = BOM;
        printf("add bom h:%d w:%d\n", h, w);
    }
}
...
void add_bom_count() {
    for (size_t y = 0; y < BD_HT; y++) {
        for (size_t x = 0; x < BD_WD; x++) {
            if (g_under_bd[y][x] == BOM) continue;
            for (size_t i = 0; i < 8; i++)
                g_under_bd[y][x] += check_mine(y + VER_HORI_OBLIQUE[i].y,
                                               x + VER_HORI_OBLIQUE[i].x);
        }
    }
}
//成功なら1,失敗なら0
...
void dig(int y, int x) {
    if (x < 0 || y < 0 || BD_WD <= x || BD_HT <= y ||
        g_ground_bd[y][x] == DUG || g_under_bd[y][x] == BOM)
        return;
    else {
        g_ground_bd[y][x] = 1;
...
int main() {
    ...
    if (init_g_values()) {
        render(false);
        while (is_game_run) main_game(&is_game_run, &game_ending);
        render(true);
    }
    ending(game_ending);
}
